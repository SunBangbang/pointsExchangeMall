<template>
			...
			<view class="recommend-info">
				<view class="recommend-title">
					<view class="gb_box">
						...
						<view class="box_gb1">
							<view class="box_gb2">温馨提醒：本商城为用户提醒兑换商城，非移动积分商城。使用会员分可抵扣现金购物！祝您生活愉快！https://shop.gougouyouxuan.com</view>
						</view>
					</view>
					<image src="/static/invest_bg.png" mode="widthFix"></image>
					
				</view>
				<view class="inve-box">
					<view class="inve-t1">
						<text class="t1a">我的{{main_name}}</text>
						<text class="t1b" @click="guize">规则</text>
					</view>
					<view class="inve-t2">
						<text class="t2a">{{userinfo.integral||0}}</text>
						<text class="t2b" @click="duihuan">点击兑换</text>
					</view>
					<view class="inve-t3">
						{{main_name}}可以兑换商品，避免失效请尽快使用
					</view>
					<view class="inve-t4">
						<view class="t4a">专属{{main_name}}<text class="t4a1">{{userinfo.integral||0}}</text></view>
						<view class="t4b">3天过期<text class="t4b1">{{userinfo.integral||0}}</text></view>
					</view>
				</view>
				<view class="clearheight300"></view>
				<view class="head-bg" :class="flextop?'flextop':''">
					<view class="session">
						<view class="list" :class="catestatus == 0 ?'action':''" @click="changecate(0,0)">
							<text class="time">限时兑换</text>
							<text class="status">超值抢兑</text>
						</view>
						<view class="list" :class="catestatus == v + 1 ?'action':''" v-for="(item,v) in cate" @click="changecate(item.id,v + 1)">
							<text class="time">{{item.title}}</text>
							<text class="status">{{item.sort_title}}</text>
						</view>
					</view>
				</view>
	...
	<view style="width: 100%;height: 300rpx;line-height: 300rpx;text-align: center;">粤ICP备2022096692号-2 </view>
	</view>
</template>
