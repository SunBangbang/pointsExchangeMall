<template>
	<view class="page">
    <!-- 付款催促 -->
    <view class="payment-urge">
      <image src="/static/icon_timing.png"></image>
      <count-down v-if="showCountDown" ref="countDownRefs" :has-hour="false" :has-millisecond="true" mode="custom" :end-timestamp="expireTimestamp"></count-down>
      <text>请在5分钟内支付，逾期订单将自动取消！</text>
    </view>
    <!-- 收货信息 -->
    ...
    <view class="goods-favorable-ratio">
      ...
      <text>恭喜！您购买的商品好评率达到{{ goods.comments }}%</text>
    </view>
		<!-- 商品 -->
      ...
      <view class="service-tag">
        ...
        <view class="list">
          <view class="degree-of-heat">
            <image src="/static/icon_exchange_gold.png"></image>
            <text>可用</text>
            <text class="highlight-color">{{ order.integral }}{{ main_name }}</text>
            <text>抵扣</text>
            <text class="highlight-color">{{ goods.old_price - order.price }}元</text>
          </view>
        </view>
    ...
    <!-- 付款提醒 -->
    <view class="payment-remind"><text>提交订单可用{{ order.integral }}{{ main_name }}抵扣现金</text></view>
		<!-- 底部合计提交 -->
